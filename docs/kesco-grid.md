...
- **Режим выбора** — включается кнопкой `CheckBox` в тулбаре. Добавляет сервисную колонку с компонентом `KescoCheckbox` (16×16px, белый фон, CSS-галочка border-rotate, tri-state с жирным квадратом для indeterminate). Выделение хранится как `HashSet<int>` (ID сущностей) и **персистентно** между страницами. Галка в заголовке управляет и `DetailRow`, и `GroupHeaderRow` (через ленивую загрузку дочерних ID → `_groupChildIds`). Сбрасывается при изменении поиска, группировки, сортировки или фильтров
