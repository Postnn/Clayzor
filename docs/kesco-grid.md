...
- **Фильтрация (tray)** — панель над гридом, скрытая по умолчанию. Открывается кнопкой `FilterAlt` в тулбаре (класс `filter-toggle-btn`). Добавление фильтра — перетаскивание заголовка на панель → открывается `KescoColumnFilterDialog`. Редактирование — клик по чипу. Удаление — клик по × на чипе. При выключении трея все фильтры сбрасываются. При любом изменении сбрасывается страница на 1
