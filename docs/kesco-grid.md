...
- **Режим выбора** — включается кнопкой `CheckBox` в тулбаре. Добавляет сервисную колонку с кастомными чекбоксами (16×16px, белый фон, классическая CSS-галочка через border-rotate, жирный квадрат для indeterminate). Выделение хранится как `HashSet<int>` (ID сущностей) и **персистентно** между страницами. Галка в заголовке управляет и `DetailRow`, и `GroupHeaderRow` (через ленивую загрузку дочерних ID → `_groupChildIds`). Сбрасывается при изменении поиска, группировки, сортировки или фильтров
...
- **`KescoGridExcelGenerator`** (сервер) — генерирует .xlsx через ClosedXML в цветах Kesco (navy `#05164D`, gold `#FFAD00`), шрифт Verdana. Поддерживает: заголовок, описание фильтров/группировки, групповые строки с Excel Outline (вложенные группы через стек по глубине — все уровни имеют +/- контролы), авто-ширину колонок. Групповые строки выводятся без иконок выбора
...
- **`KescoGridPrintHtmlGenerator.Build()`** (сервер) — строит самодостаточный HTML с инлайн-стилями (Ч/Б-таблица, тёмный header, `@page{landscape;15mm}`, `thead{display:table-header-group}`). Групповые строки выводятся без иконок разворота/сворачивания и выбора — только имя группы и количество
