...
| `ShowFilterTray` | `bool` | `false` | Включить drag‑and‑drop tray для фильтрации по колонкам |
| `AvailableFilterColumns` | `Dictionary<string, string>` | `[]` | Доступные для фильтрации колонки: ключ — отображаемое имя, значение — SQL-имя колонки |
| `FilterColumnTypes` | `Dictionary<string, ColumnType>` | `[]` | Тип данных каждой фильтруемой колонки: ключ — SQL-имя, значение — `ColumnType.Text`/`Number`/`Boolean` |
...
<KescoGrid TEntity="IGridRow"
           ...
           ShowFilterTray="true"
           AvailableFilterColumns="@_filterColumnsDef"
           FilterColumnTypes="@_filterColumnTypes"
...
@code {
    ...
    private static readonly Dictionary<string, string> _groupColumnsDef = new()
    {
        ["Название"] = "НазваниеКолонки",
    };
    ...
    private static readonly Dictionary<string, ColumnType> _filterColumnTypes = new()
    {
        ["SqlCol"]           = ColumnType.Number,
        ["НазваниеКолонки"]  = ColumnType.Text,
    };
    private static readonly Dictionary<string, string> _filterFlatColumnMap = new()
    {
