...
/// <summary>
/// JSON-конвертер для полиморфного дерева фильтра (<see cref="IKescoFilterNode"/>).
/// Использует дискриминатор <c>$type</c>:
/// <c>"group"</c> → <see cref="KescoFilterGroupNode"/>,
...
/// </summary>
public sealed class KescoFilterJsonConverter : JsonConverter<IKescoFilterNode>
{
    private const string TypeDiscriminator = "$type";

    public override IKescoFilterNode? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        if (reader.TokenType != JsonTokenType.StartObject)
            throw new JsonException("Expected StartObject for IKescoFilterNode");
    ...
    public override void Write(Utf8JsonWriter writer, IKescoFilterNode value, JsonSerializerOptions options)
    {
    ...
    private static void WriteGroup(Utf8JsonWriter writer, KescoFilterGroupNode group)
    {
    ...
    private static KescoFilterGroupNode DeserializeGroup(JsonElement root)
    {
        var group = new KescoFilterGroupNode();
        ...
        if (root.TryGetProperty("nodes", out var nodesProp) && nodesProp.ValueKind == JsonValueKind.Array)
        {
            ...
            {
                var node = JsonSerializer.Deserialize<IKescoFilterNode>(nodeEl.GetRawText());
                if (node is not null)
    ...
    private static ColumnFilter DeserializeColumnFilter(JsonElement root)
    {
        ...
        if (root.TryGetProperty("source", out var srcProp))
            leaf.Source = (KescoFilterSource)srcProp.GetInt32();
