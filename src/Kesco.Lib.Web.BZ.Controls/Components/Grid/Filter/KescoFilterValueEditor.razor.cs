...
/// Переиспользуется в <c>KescoColumnFilterDialog</c> и в будущем диалоге составного фильтра (задача 09).
/// При операторах без значения (IsEmpty/IsNotEmpty/IsNull/IsNotNull) скрывается и обнуляет <see cref="Value"/>.
/// </summary>
public partial class KescoFilterValueEditor : ComponentBase
{
    // ── Параметры ──────────────────────────────────────────────────────────────
    ...
    /// </summary>
    [Parameter]
    public IReadOnlyList<KescoFilterOption>? Options { get; set; }
