...
Файлы: `Components/Grid/KescoGrid.razor`,
`Components/Grid/Filter/KescoFilterDialog.razor`.
...
`kesco-grid-print-descriptions` (в `KescoGrid.razor`, ~строки 152–167), который выводит
...
стиль приложения или `<style>` в `KescoGrid.razor` — класс грид-специфичный):
```css
.kesco-grid-print-descriptions { display: none; }
@media print {
    .kesco-grid-print-descriptions { display: block; }
}
```
...
Причина: скролл-override завязан на `.kesco-filter-dialog .mud-dialog-content`, но
...
Переписать `KescoFilterDialog.razor` проще (без своего `<style>`, без внутренней
...
```
Убрать: `Class="kesco-filter-dialog"`, весь блок `<style>…</style>`, внутренний
