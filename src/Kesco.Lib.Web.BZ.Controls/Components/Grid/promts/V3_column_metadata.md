...
## Файлы
- `Components/Grid/KescoColumnDef.razor` — новые `[Parameter]`.
- `Components/Grid/IKescoGrid.cs` — поля в `KescoColumnMeta` + расширение
  сигнатуры `RegisterColumn`.
- `Components/Grid/KescoGrid.razor.cs` — реализация `RegisterColumn`
  (метод строит `KescoColumnMeta`, ищи по `_columnBySqlName[sqlName] = meta`).
- `Components/Grid/KescoColumn.razor` вызывать не нужно (регистрация идёт из
  `KescoColumnDef.OnInitialized`).

## `KescoColumnDef.razor` — добавить параметры
...
## `IKescoGrid.KescoColumnMeta` — добавить init-поля
...
Заполнить ими новые поля `KescoColumnMeta`.

## Глобальный тумблер грида (треб. 1 — «включать в настройках грида»)
- В `KescoGrid.razor.cs` добавить `[Parameter] public bool EnableValueFilter { get; set; } = true;`
...
- Тумблер в диалоге настройки колонок (`KescoColumnSettingsDialog`) — **не здесь**,
  это опциональная задача V8.

## Критерии
- [ ] Существующие вызовы `RegisterColumn` компилируются без правок (новые
      параметры опциональны).
- [ ] `KescoColumnMeta` несёт `AllowValueFilter`, `BoolTrueLabel`,
      `BoolFalseLabel`.
- [ ] `KescoColumnDef` прокидывает новые параметры в реестр.
