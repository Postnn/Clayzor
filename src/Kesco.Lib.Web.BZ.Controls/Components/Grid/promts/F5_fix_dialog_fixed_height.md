...
Файлы: `Components/Grid/Filter/KescoFilterDialog.razor`,
`Components/Grid/KescoGrid.Filtering.cs`.
...
### 1. Опции диалога (`KescoGrid.Filtering.cs`, `OpenCompositeFilterDialog`)
...
### 2. Разметка `KescoFilterDialog.razor` — фиксированная высота + 3 зоны
```razor
<MudDialog Class="kesco-filter-dialog">
    <TitleContent>
    ...
    <DialogContent>
        ...
        <div style="display:flex;flex-direction:column;height:520px;width:560px;max-width:100%">
            ...
            <div style="flex:1 1 auto;overflow-y:auto;min-height:0">
                <KescoFilterGroup Node="@_draft" Columns="@Columns"
...
<style>
    ...
    .kesco-filter-dialog .mud-dialog-content { overflow: hidden; }
</style>
```
...
- `.kesco-filter-dialog .mud-dialog-content { overflow:hidden }` — чтобы скролл был
...
### 1. `KescoFilterExpression.razor` — всё условие в одну строку
...
<div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;
    ...
    <MudSelect T="ColumnFilterOperator" Label="Условие" Value="@Node.Operator"
        ...
        {
            <MudSelectItem Value="@op">@KescoFilterOperatorLabels.Get(op)</MudSelectItem>
        }
    </MudSelect>
    ...
    @if (_descriptor is not null)
    {
        <div style="flex:1 1 130px;min-width:120px">
            <KescoFilterValueEditor Type="@_descriptor" Value="@Node.Value"
...
### 2. `KescoFilterGroup.razor` — тоньше отступы
...
### 3. `KescoFilterDialog.razor` — плотнее рабочая область
