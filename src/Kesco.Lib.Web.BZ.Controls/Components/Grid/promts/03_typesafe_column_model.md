...
редакторы в `KescoColumnFilterDialog` (enum→контрол), `KescoDataQuery.BuildSingleClause`
...
- `KescoColumnMeta` += `public ColumnTypeDescriptor Type { get; init; }`;
  грид резолвит дескриптор при регистрации колонки.
- `KescoGridPageBase.MapClrTypeToColumnType` → делегирует в `ColumnTypeRegistry.FromClr`.
  `FilterColumnTypes` оставить как **производное** (`SqlName→Kind`) для совместимости.
- `KescoDataQuery.BuildSingleClause` — оставить один `switch` по **оператору**
...
- [ ] `KescoColumnMeta.Type` — единственный источник; `FilterColumnTypes` производный.
