# V6. Компонент диалога фильтра по значению `KescoColumnValueFilterDialog`
...
`KescoColumnFilterDialog`. Файлы: `Components/Grid/KescoColumnValueFilterDialog.razor`
...
- либо, при клике на условие, диалог закрывается с сигналом «открыть форму
  ...
  маршрутизацию (открыть `KescoColumnFilterDialog` с `InitialOperator=X`) делает V7.
...
1. **Шапка — контекстный список условий (треб. 7).** Раскрывающийся пункт
   ...
   `_descriptor.Operators` с подписями `KescoFilterOperatorLabels.Get(op)`
   (как в `KescoColumnFilterDialog.GetOperatorLabel`). Клик по оператору →
...
2. **Блок состояния (условно).**
   - Если `ExistingConditionFilter != null` (треб. 8): показать плашку
     ...
     `KescoColumnFilterDialog.GetFilterDescription(ExistingConditionFilter, ColumnDisplayName)`)
...
## Переиспользование
- Подписи операторов: `KescoFilterOperatorLabels.Get` (см.
  `Filter/KescoFilterOperatorLabels.cs`).
- Формат значений: `ColumnTypeDescriptor.Format`.
- Стиль/плотность списка и `max-height` popover — по образцу `<style>` в
  `KescoColumnFilterDialog.razor` (`.mud-list-item{min-height:32px}` и т.п.).

## Критерии
- [x] Значения грузятся **только** в `OnInitializedAsync` (лениво), с индикатором
      и курсором ожидания (12).
- [x] `Capped` → сообщение «недоступно, > 100», список не рендерится (3, 10).
- [x] «(Выделить все)» — sticky, не прокручивается; «(пустые)» — в конце при
      наличии пустышек (6); каждое значение с чекбоксом (5); высота фиксирована,
      скроллится только зона значений (4).
- [x] Контекстный список условий по типу колонки; клик → возврат
      `OpenConditionRequest(op)` (7).
- [x] При активном фильтре по условию: плашка с описанием + удаление, значения
      заблокированы (8, 9).
- [x] bool-подписи берутся из `BoolTrueLabel`/`BoolFalseLabel` (15).
- [x] OK собирает `ValueFilter` с выбором меньшей стороны и `Negate` (14);
