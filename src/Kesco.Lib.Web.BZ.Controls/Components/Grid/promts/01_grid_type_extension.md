...
- Колонки на странице объявляются `KescoColumnDef` **без типа данных**
...
- Тип фильтра берётся из `FilterColumnTypes: IReadOnlyDictionary<string, ColumnType>`,
  ...
  Вычисляется рефлексией в `KescoGridPageBase.InferFilterColumnTypes()` / `MapClrTypeToColumnType`.
...
- Редактор `Number` в `KescoColumnFilterDialog` — `MudNumericField<int?>`.
...
- `KescoColumnFilterDialog`: редактор `MudDatePicker` (хранить `DateTime? _date1/_date2`), ветки в `RestoreValue/GetValue/ClauseHasValue`; формат отображения `dd.MM.yyyy`.
...
- `KescoColumnFilterDialog`: редактор `MudNumericField<decimal?>` (`_dec1/_dec2`); ветки в `RestoreValue/GetValue/ClauseHasValue`.
...
- Источник вариантов — необязательный маппинг на странице/в базовом классе:
  `protected virtual IReadOnlyDictionary<string, IReadOnlyList<KescoFilterOption>> FilterLookupOptions`
  (ключ — SqlName; `KescoFilterOption { object? Value; string Label; }`).
...
- Если для колонки задан список вариантов — редактор значения рендерит выпадающий
  список (по образцу существующего `KescoComboBox`/`ILookupEntity`) вместо `MudTextField`/`MudNumericField`.
