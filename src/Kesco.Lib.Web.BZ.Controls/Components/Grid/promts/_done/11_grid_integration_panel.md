# 11. Интеграция в KescoGrid: панель и маршрутизация
...
Правки идут в `KescoGrid.Filtering.cs` (задача 05).

## Колоночный фильтр → лист дерева
- `OnFilterTrayDrop`/`OpenFilterDialog(sqlName, displayName)`: как сейчас открывают
  `KescoColumnFilterDialog` через `DialogService.ShowExAsync<...>`, получают `ColumnFilter`.
  Новое: проставить `Source = KescoFilterSource.ColumnDialog` и **вставить/заменить**
  лист в дереве по `Column` (есть лист этой колонки → заменить, иначе добавить в корень).
- Редактирование: клик по сегменту колоночного листа → `KescoColumnFilterDialog`
  с `ExistingFilter = <этот ColumnFilter>`.

## Диалог настраиваемого фильтра
- `OpenCompositeFilterDialog()` (объявлен в задаче 10):
  `DialogService.ShowExAsync<KescoFilterDialog>(...)`, передать `_filterRoot.Clone()`,
...
- Заменить рендер чипов: рекурсивный обход `_filterRoot` через **единый** построитель
  описания (расширить `KescoColumnFilterDialog.GetFilterDescription` на группы: скобки, И/ИЛИ).
