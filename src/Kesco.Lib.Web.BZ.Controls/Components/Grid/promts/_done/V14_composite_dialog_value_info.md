...
установлен фильтр по значению. Причина: `KescoFilterDialog` клонирует весь
`_filterRoot` в `_draft` (узлы `ValueFilter` там есть и на «Применить»
сохраняются — не теряются), но `Filter/KescoFilterGroup.razor` перебирает
`Node.Nodes` и имеет ветки только для `ColumnFilter` и `KescoFilterGroupNode`;
...
Файл: `Components/Grid/Filter/KescoFilterGroup.razor`.

## Изменение
В цикле `@for (int i = 0; i < Node.Nodes.Count; i++)`, после ветки
`else if (node is KescoFilterGroupNode group) { … }`, добавить:
```razor
...
- Изменения в `KescoFilterDialog.razor(.cs)` не нужны: `_draft` — клон всего
