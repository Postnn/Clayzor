...
Файлы: `Components/Grid/KescoGrid.Filtering.cs`, `Components/Grid/KescoGrid.razor`.
...
**Фикс.** `ValueFilter` — не составной. В `KescoGrid.Filtering.cs`:
```csharp
...
### 1. Хелпер листьев (рядом с `ColumnDialogLeaves`) в `KescoGrid.Filtering.cs`
```csharp
...
### 2. Чипы значения в трее (`KescoGrid.razor`, блок `@if (_filterTrayExpanded)`)
