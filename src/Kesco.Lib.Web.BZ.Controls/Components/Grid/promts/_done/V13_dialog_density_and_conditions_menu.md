...
Две правки в `Components/Grid/KescoColumnValueFilterDialog.razor`:
...
> `Components/KescoMenu.razor`: `<ActivatorContent Context="x">` →
...
```razor
<div class="kesco-vf-dialog" style="display:flex;flex-direction:column;gap:6px;min-width:300px">
```

### Стили (заменить блок `<style>` в этом файле)
Скопить строки и ужать отступы. Правила скоупить под `.kesco-vf-dialog`, чтобы не
задевать другие диалоги:
```razor
<style>
    .kesco-vf-dialog { font-size: .875rem; }
    /* компактные строки чекбоксов (KescoCheckbox после V11 или span-строки после V10) */
    .kesco-vf-dialog .kesco-checkbox { padding: 1px 0 !important; gap: 6px; }
    /* длинные подписи — в одну строку с многоточием */
    .kesco-vf-dialog .kesco-checkbox > span:last-child {
        white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 340px;
    }
    /* прокрутка значений — фикс. высота, чуть ниже */
    .kesco-vf-dialog .value-filter-values-scroll { overflow-y: auto; max-height: 260px; }
    .kesco-vf-dialog .value-filter-select-all {
...
```
Если после V11 чекбоксы — это `KescoCheckbox` (класс `.kesco-checkbox`), правила
выше подходят. Если ещё span-строки из V10 — они тоже внутри `.kesco-checkbox`? Нет:
в V10 у строк был свой inline-стиль. Тогда в V10-разметке добавить класс
`kesco-checkbox` на строку-обёртку `<div>`, либо снизить inline `padding:4px 0`
до `padding:1px 0`. Рекомендуется сначала выполнить V11 (компонент), тогда
селектор `.kesco-checkbox` работает единообразно.
...
- [ ] Правила уплотнения не протекают на другие диалоги (скоуп `.kesco-vf-dialog`).
