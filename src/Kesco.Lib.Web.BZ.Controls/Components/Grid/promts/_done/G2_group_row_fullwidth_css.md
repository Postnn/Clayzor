...
Файлы:
- `Components/Grid/KescoGrid.razor` — `CellClassFunc` на колонке чекбокса, колонке редактирования и
...
**Не трогаем** `Services/KescoGridPrintHtmlGenerator.cs` — у него собственный `<style>`, встроенный в
...
(в отличие от печати, где `KescoGridPrintHtmlGenerator` честно ставит этот класс на `<td colspan="N">`).
...
### 1. `CellClassFunc` в `KescoGrid.razor`
...
<TemplateColumn @key="SelectColumnKey" T="TEntity"
                ...
                CellClassFunc="@(item => item is GroupHeaderRow ? "kesco-group-pin-cell" : "")"
...
/* ── Group Header Rows: полная ширина в интерактивном гриде ──
   ...
   см. KescoGridPrintHtmlGenerator.AppendGroupRow, там честный colspan).
   ...
   запиненной колонки чекбокса (.kesco-group-pin-cell). */
.mud-table-row:has(.group-header-cell) {
    display: flex;
    align-items: stretch;
}

.mud-table-row:has(.group-header-cell) > .mud-table-cell:not(.group-header-cell):not(.kesco-group-pin-cell) {
...
.mud-table-row:has(.group-header-cell) .kesco-group-pin-cell {
    flex: 0 0 auto;
}
```
...
Ручной отступ на уровень вложенности не трогаем — он уже есть в `KescoGroupHeader.razor`
...
- [ ] Строка-заголовок группы визуально тянется на всю ширину таблицы (сравнить с тем, как это уже
      выглядит в печатной форме — `KescoGridPrintHtmlGenerator`).
...
- [ ] Печатная форма (`KescoGridPrintHtmlGenerator`) не изменилась внешне — её стили независимы.
