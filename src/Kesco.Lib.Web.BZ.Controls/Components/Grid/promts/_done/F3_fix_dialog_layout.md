...
`Components/Grid/Filter/KescoFilterExpression.razor`,
`Components/Grid/Filter/KescoFilterGroup.razor`,
`Components/Grid/KescoGrid.Filtering.cs` (опции диалога).
...
1. Опции диалога (`KescoGrid.Filtering.cs`, метод `OpenCompositeFilterDialog`):
   ```csharp
...
3. Уменьшить накопление отступа вложенности в `KescoFilterGroup.razor`: для не-корневых
