...
Файлы: `Components/Grid/Filter/KescoFilterGroup.razor.cs`,
`Components/Grid/Filter/KescoFilterDescriptionBuilder.cs`.
...
и **в SQL используются оба** (`KescoCompositeSqlBuilder.BuildLeaf` учитывает
...
`KescoFilterGroup` берёт лишь `leaf.Operator`/`leaf.Value` и игнорирует второе условие.

## Исправить (и убрать дублирование)
Логика описания листа с двумя клаузами уже есть в `KescoFilterDescriptionBuilder`
(приватный `BuildLeafText`). Переиспользовать её вместо отдельной реализации.

1. В `KescoFilterDescriptionBuilder` открыть публичный метод описания одного листа:
   ```csharp
...
2. В `KescoFilterGroup.razor.cs` заменить тело `GetLeafDescription` на вызов общего метода
