...
`Components/Grid/KescoGrid.razor` (трей фильтра),
`Components/Grid/Filter/KescoFilterExpression.razor(.cs)`.
...
`KescoFilterDialog.razor` — перенести фиксированную высоту на сам content (scoped),
...
`KescoGrid.razor`, блок трея (`@if (_filterTrayExpanded)`):
```razor
@if (_hasComposite)
{
