# V2. SQL для `ValueFilter` в `KescoCompositeSqlBuilder`
...
## Где
`Filter/KescoCompositeSqlBuilder.cs`. Сейчас `BuildGroup` разбирает узлы через
`switch`: `KescoFilterGroupNode` → `BuildGroup`, `ColumnFilter` → `BuildLeaf`,
