...
Файлы: `Components/Grid/KescoGrid.razor` (блок трея фильтра),
при необходимости `Components/Grid/KescoGrid.Filtering.cs`.
Закрывает баг 4.
...
## Реализация (KescoGrid.razor, блок `@if (_filterTrayExpanded)`)
...
## Хелперы (KescoGrid.Filtering.cs)
```csharp
...
{
    if (seg.Source == KescoFilterSource.ColumnDialog)
    {
...
@foreach (var seg in _filterSegments)
{
    ...
    @if (s.Source == KescoFilterSource.ColumnDialog)
    {
        <MudIconButton Icon="@Icons.Material.Filled.Close" Size="Size.Small"
...
```
`RemoveFilter(sqlName)` уже есть в `KescoGrid.Filtering.cs`. Составные условия
