...
Файл: `Components/Grid/KescoColumnValueFilterDialog.razor`.

## Эталон — из `KescoGrid.razor` (сервисная колонка выбора)
...
инлайн-повтор тех же трёх `<span>`-вариантов в каждом месте (как в `KescoGrid.razor`).
...
- [ ] В диалоге нет ни одного `MudCheckBox`; чекбоксы — кастомные `<span>` как в
      `KescoGrid.razor` (тот же вид, включая «частично» у «Выделить все»).
