...
интерфейс `Filter.IKescoFilterNode` (см. `Filter/IKescoFilterNode.cs`), как это
уже делает `ColumnFilter` (см. `KescoDataQuery.cs`).

## Класс `ValueFilter : IKescoFilterNode`
Поля:
...
## Важно
- Ничего не менять в `KescoCompositeSqlBuilder` — узел там пока просто попадёт в
  ветку `_ => null` (это ожидаемо до V2, дерево остаётся валидным).
- XML-док на русском на класс и все публичные члены (стиль как в
  `KescoFilterGroupNode.cs` / `ColumnFilter`).

## Критерии
- [ ] `ValueFilter` реализует `IKescoFilterNode`, лежит в неймспейсе
      `Kesco.Lib.Web.BZ.Controls.Components.Grid.Filter`.
- [ ] `Clone()` — независимая глубокая копия (правка копии не трогает оригинал,
      в т.ч. список `Values`).
- [ ] `HasValue` учитывает и значения, и `BlankChecked`.
- [ ] `KescoCompositeSqlBuilder` и `ColumnFilter` не изменены.
