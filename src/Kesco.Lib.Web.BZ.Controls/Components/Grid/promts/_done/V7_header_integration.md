...
`KescoGrid.razor` в установившемся состоянии рендерит заголовки **не** через
`KescoColumn.HeaderTemplate`. Две ветки в `<Columns>`:
- `@Columns` — только пока `!_columnsReady` (fallback/во время загрузки): это
  компоненты `KescoColumn` с их `HeaderTemplate`.
...
загрузки). Приоритет — инлайновая шапка в `KescoGrid.razor`.

### Инлайновая шапка в `KescoGrid.razor` (обязательно)
...
```razor
@if (((IKescoGrid)this).IsValueFilterAvailable(sqlName))
{
    var vfSql    = sqlName;
    var vfActive = ((IKescoGrid)this).IsValueFilterActive(vfSql);
    <MudIconButton Icon="@Icons.Material.Filled.FilterList"
                   Size="Size.Small"
                   Color="@(vfActive ? Color.Primary : Color.Default)"
                   OnClick="async () => await ((IKescoGrid)this).OpenValueFilterDialog(vfSql)"
                   Style="padding:0;width:22px;height:22px;min-width:22px;margin-right:2px"
                   title="Фильтр по значению" />
}
```
Вызовы — через `((IKescoGrid)this).…` (это явные реализации интерфейса, как уже
...
для `.kesco-column-menu`). На кнопке:
`Class="@("kesco-vf-icon" + (vfActive ? " kesco-vf-icon--active" : ""))"`.
Добавить в `app.css` рядом с правилом `.kesco-column-menu` (подсветка активного —
треб. 13):
```css
.kesco-vf-icon .mud-svg-icon path:not([fill="none"]) { fill: var(--lh-white) !important; opacity: .55; }
.mud-table-cell:hover .kesco-vf-icon .mud-svg-icon path:not([fill="none"]) { opacity: 1; }
.kesco-vf-icon--active .mud-svg-icon path:not([fill="none"]) { fill: var(--lh-gold) !important; opacity: 1; }
```

### Значок в fallback-шапке (`KescoColumn.razor`)
...
- [ ] Значок добавлен в ОБА пути отрисовки шапки: инлайновый `HeaderTemplate` в
      `KescoGrid.razor` (цикл по `_columnOrder`, основной) И `KescoColumn.razor`
