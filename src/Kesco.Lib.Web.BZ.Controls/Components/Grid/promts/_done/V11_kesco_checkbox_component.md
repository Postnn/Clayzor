# V11. Компонент `KescoCheckbox` (вынос кастомного чекбокса)

Кастомный `<span>`-чекбокс (как в выборе записей грида) сейчас дублируется в
нескольких местах: шапка выбора, строка группы, строка-деталь в `KescoGrid.razor`
и три места в `KescoColumnValueFilterDialog.razor` (после V10). Вынести в
переиспользуемый компонент `KescoCheckbox` и заменить все вхождения.
...
## Соглашения (как `KescoButton`/`KescoMenu`)
Один файл `.razor` с `@code`, RU-докстринги на публичные члены, неймспейс
`Kesco.Lib.Web.BZ.Controls.Components`. Компонент — «управляемый» (controlled):
...
## Новый файл `Components/KescoCheckbox.razor`
```razor
@namespace Kesco.Lib.Web.BZ.Controls.Components

<div class="kesco-checkbox @Class"
...
- Если `_Imports.razor` не подключает `...Components`, добавить `@using` или
  вызывать как `<KescoCheckbox>` там, где `KescoButton`/`KescoMenu` уже видны.

## Рефактор вызовов в `KescoGrid.razor`
...
<div style="display:flex;align-items:center;justify-content:center;width:100%">
    <KescoCheckbox State="@(IsHeaderIndeterminate() ? null : (bool?)_selectAllChecked)"
...
<KescoCheckbox State="@(gs.Indeterminate ? null : (bool?)gs.Checked)"
...
```razor
<KescoCheckbox State="(bool?)detailChecked"
...
## Рефактор `KescoColumnValueFilterDialog.razor` (заменяет разметку из V10)
Удалить локальный шаблон `glyph` и три блока `<div @onclick=…>@glyph(...)…`.
Вместо них:
```razor
<div class="value-filter-select-all">
    <KescoCheckbox State="IsSelectAllState"
                   Label="(Выделить все)"
...
<div class="value-filter-values-scroll">
    ...
    {
        var captured = value;
        <KescoCheckbox State="(bool?)_checkedValues.Contains(captured)"
    ...
    {
        <KescoCheckbox State="(bool?)_blankChecked"
                       Label="(пустые)"
                       Style="padding:4px 0"
...
## Критерии
- [ ] Создан `Components/KescoCheckbox.razor` (controlled: `State` + `OnToggle`,
      tri-state как в гриде, опц. `Label`/`Title`/`Disabled`/`Class`/`Style`).
- [ ] Все 3 места в `KescoGrid.razor` используют `KescoCheckbox`; вид и поведение
      выбора записей/групп не изменились.
- [ ] Диалог значения использует `KescoCheckbox` вместо inline-`<span>`; галочки и
