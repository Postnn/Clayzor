...
`Components/Grid/Filter/KescoFilterGroup.razor`,
...
1. `KescoGrid.Filtering.cs`, `OpenCompositeFilterDialog` — убрать перетаскиваемый режим
   ...
   ```
2. `KescoFilterDialog.razor` — задать диалогу класс и **scoped** (не глобальный!) стиль,
   ...
   <style>
       /* Ограничено префиксом .kesco-filter-dialog — на другие диалоги не влияет */
       .kesco-filter-dialog .mud-dialog { max-height: 90vh; }
       .kesco-filter-dialog .mud-dialog-content {
...
Причина: в `KescoFilterGroup.razor` тумблер — `MudButtonGroup Variant="Outlined"`,
...
- [ ] Никаких глобальных (нескоупленных) стилей — только под `.kesco-filter-dialog`.
