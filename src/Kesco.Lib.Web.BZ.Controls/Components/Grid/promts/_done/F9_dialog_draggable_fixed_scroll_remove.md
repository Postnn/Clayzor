...
- Override `.kesco-filter-dialog .mud-dialog-content` не срабатывал (класс уходил не на
...
### `KescoGrid.Filtering.cs` — вернуть DragMode составному диалогу
```csharp
...
### `KescoFilterDialog.razor` — ContentStyle + прямые flex-дети
```razor
...
Сейчас лист `Source=ColumnDialog` в `KescoFilterGroup.razor` рисуется read-only блоком
«Редактируется в диалоге колонки: …» без кнопки удаления. Добавить крестик.

Найти блок (ветка `leaf.Source == KescoFilterSource.ColumnDialog`) и добавить удаление
