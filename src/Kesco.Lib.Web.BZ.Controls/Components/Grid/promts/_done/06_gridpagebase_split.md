# 06. Разбиение KescoGridPageBase на partial-классы

`KescoGridPageBase.cs` — 1157 строк, смешивает загрузку, экспорт, печать, вывод
типов. Разнести по partial-файлам **без изменения поведения**. Все файлы:
`public partial class KescoGridPageBase<T> ...` — повторить точную сигнатуру и
ограничения (`where T : ...`); базовый класс/интерфейсы — только в основном файле.

## Раскладка (методы реальные)
`KescoGridPageBase.cs` (ядро):
...
`KescoGridPageBase.ColumnTypes.cs` (вывод типов; согласовать с задачей 03):
...
`KescoGridPageBase.Export.Excel.cs`:
- `IKescoGridDataLoader.ExcelExportAsync`, `BuildAllRowsForExcel`,
  `BuildAllGroupedRowsForExcel`, `BuildExportRows`, `CollectCounts`, `SanitizeFileName`.

`KescoGridPageBase.Export.Print.cs`:
- `BuildAllRowsForPrint`, `BuildAllFlatRowsForPrint`, `BuildAllGroupedRowsForPrint`.

`KescoGridPageBase.Export.Selected.cs`:
