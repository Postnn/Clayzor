...
Файлы:
- `Components/Grid/KescoGrid.razor` — сервисные колонки чекбокса/редактирования + цикл колонок данных.
- `Components/Grid/KescoGrid.razor.cs` или `KescoGrid.Grouping.cs` — новый параметр `OnGroupToggle`,
  вычисление хост-колонки.
- `Components/Grid/KescoGroupHeader.razor` — без изменений в разметке, меняется только место вызова.
- Страницы-потребители вне `Kesco.Lib.Web.BZ.Controls` (в основном веб-проекте решения), которые сегодня
  вручную кладут `<KescoGroupHeader>` внутрь `CellTemplate` одной из колонок — найти по
  `grep -rn "KescoGroupHeader" --include=*.razor` и `grep -rn "is GroupHeaderRow"`.
...
По соглашению это обычно «первая» колонка (см. doc-comment в `KescoGroupHeader.razor`). Отсюда два
взаимосвязанных дефекта:

1. Сервисные колонки — чекбокс выбора (`KescoGrid.razor`, блок `_selectMode`) и колонка редактирования
...
2. Колонка, которая скрывается при группировке по ней (`Hidden="@IsGrouped(sqlName)"` — см.
   `KescoColumn.razor._hidden` и цикл в `KescoGrid.razor`), очень часто оказывается той же колонкой, в
   `CellTemplate` которой лежит `<KescoGroupHeader>`. Стоит сгруппировать по ней — и единственное место,
...
3. Чекбокс выбора — не хост, у него отдельная роль: при `_selectMode == true` для `GroupHeaderRow` он,
   ...
   `OnGroupTriToggle` в `KescoGrid.Grouping.cs`) — это уже работает, не трогаем.

## Правки

### 1. Вычисление хост-ключа и новый параметр

Разместить рядом с остальным кодом группировки (`KescoGrid.Grouping.cs`) — один источник истины:

```csharp
/// <summary>
/// Событие переключения раскрытия/сворачивания группы.
/// Страница-потребитель подписывается через `OnGroupToggle="ToggleGroup"` на теге &lt;KescoGrid&gt;.
...
`KescoGrid.razor`, блок `EditDialogType is not null`:
...
<CellTemplate>
    @if (context.Item is GroupHeaderRow gh)
    {
        ...
        {
            <KescoGroupHeader Header="gh" OnToggle="OnGroupToggle" />
        }
    }
    else if (context.Item is IDetailRow detail)
    {
        <KescoButton Icon="@Icons.Material.Filled.Edit"
                     Text="Редактировать"
                     Size="Size.Small"
...
`KescoGrid.razor`, `@foreach (var colId in _columnOrder)`. Перед вызовом `cellTemplate(context)`
...
На страницах вне `Kesco.Lib.Web.BZ.Controls`, которые сегодня вручную вставляют

```razor
@if (context.Item is GroupHeaderRow header) { <KescoGroupHeader Header="header" OnToggle="ToggleGroup" /> }
```

внутрь `CellTemplate` какой-то одной колонки:

1. Убрать оттуда явную вставку `<KescoGroupHeader>` — это теперь делает грид сам.
2. На теге `<KescoGrid ...>` этой страницы добавить `OnGroupToggle="ToggleGroup"`.
...
- [ ] На страницах-потребителях не осталось ручной вставки `<KescoGroupHeader>` внутри `CellTemplate` —
      управление полностью в `KescoGrid`, подписка только через `OnGroupToggle`.
