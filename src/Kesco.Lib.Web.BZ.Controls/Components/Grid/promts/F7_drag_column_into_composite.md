...
Файлы: `Components/Grid/KescoGrid.Filtering.cs`,
`Components/Grid/Filter/KescoFilterExpression.razor(.cs)`,
`Components/Grid/KescoDataQuery.cs` (транзиентный флаг на `ColumnFilter`).
...
{
    var sql = KescoDragState.DraggedColumn;
...
private KescoFilterGroupNode BuildTreeWithColumnAnded(string sql)
{
...
## Транзиентный флаг фокуса на `ColumnFilter` (KescoDataQuery.cs)
```csharp
...
## Автофокус нового условия (KescoFilterExpression)
