...
(меню «Групповые операции» → «Печать» / «Выгрузка в Excel» в `KescoGrid.razor`,
обработчики в `KescoGrid.ExportMenu.cs`) — спрашивать пользователя, нужно ли
...
Зависит от существующих: `KescoColumnSettingsDialog.razor`, `ColumnSettingsItem.cs`,
`IKescoGrid.GetVisibleColumns()`, `KescoGrid.ExportMenu.cs`.

## Точки входа (6 штук, все в `KescoGrid.ExportMenu.cs`)
...
`var columns = ((IKescoGrid)this).GetVisibleColumns();`. Эту строку нужно заменить
...
Сделать новый лёгкий диалог `Components/Grid/KescoColumnSettingsPromptDialog.razor`:
...
- открывается через `DialogService.ShowExAsync<KescoColumnSettingsPromptDialog>(...)`
  с `DialogOptionsEx { MaxWidth = MaxWidth.ExtraSmall, FullWidth = true }` — тем же
  паттерном, что и `KescoColumnSettingsDialog`.

## 2. `KescoColumnSettingsDialog.razor` — режим без сортировки
Добавить параметр:
```csharp
...
`OpenColumnSettings()` в `KescoGrid.razor.cs` не трогаем.
...
Важно: работать нужно с **копией** — как и в `KescoColumnSettingsDialog.OnInitialized`,
никакие правки в диалоге не должны попадать в `_columnOrder` / `_hiddenSqlNames`
самого грида.

## 4. Общий метод разрешения колонок
В `KescoGrid.ExportMenu.cs` добавить:
```csharp
private async Task<IReadOnlyList<KescoColumnMeta>?> ResolveExportColumnsAsync(string contextLabel)
```
Логика:
1. Открыть `KescoColumnSettingsPromptDialog` с `ContextLabel = contextLabel`.
2. Результат `Canceled` → вернуть `null`.
3. Результат `false` («как на странице») → вернуть `((IKescoGrid)this).GetVisibleColumns()`.
4. Результат `true` («настроить») → построить `BuildColumnSettingsItems()`, открыть
   `KescoColumnSettingsDialog` с `ShowSorting = false` и подходящим заголовком.
   ...
   - Если применил — из результирующего `List<ColumnSettingsItem>` взять элементы с
     ...
     и вернуть как `IReadOnlyList<KescoColumnMeta>`.
   - Если пользователь снял видимость со всех колонок — не давать применить пустой
     результат: заблокировать кнопку «Применить» в `KescoColumnSettingsDialog` (или
...
`var columns = ((IKescoGrid)this).GetVisibleColumns();` на
```csharp
...
он уже принимает `IReadOnlyList<KescoColumnMeta>` в нужном порядке и ничего не знает,
...
- `kescoColumnSettings.js`, `kescoGridPrint.js`, `kescoGridExcel.js` — без изменений,
