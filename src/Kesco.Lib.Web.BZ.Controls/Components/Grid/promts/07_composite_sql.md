...
Построить `WHERE` из дерева `KescoFilterGroupNode` (задача 02) рекурсивно,
...
## Подготовка существующего кода (`KescoDataQuery.cs`)
...
## Новый файл `Components/Grid/Filter/KescoCompositeSqlBuilder.cs`
```csharp
public static class KescoCompositeSqlBuilder
{
    ...
    //   вызвать KescoDataQuery.BuildSingleClause.
}
```
