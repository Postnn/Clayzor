...
public static class KescoGridPrintHtmlGenerator
{
        ...
        string? groupDescription = null)
    {
        ...
        // Обёртка с классом kesco-grid-printing — активирует @media print CSS
        sb.Append("<div class=\"kesco-grid-printing mud-paper\" style=\"position:relative\">");
        ...
        {
            sb.Append("<div class=\"kesco-grid-print-descriptions\">");
            if (!string.IsNullOrWhiteSpace(groupDescription))
            {
                sb.Append("<div class=\"kesco-grid-print-group-desc\">")
                  .Append(EscapeHtml(groupDescription)).Append("</div>");
            }
            if (!string.IsNullOrWhiteSpace(filterDescription))
            {
                sb.Append("<div class=\"kesco-grid-print-filter-desc\">")
    ...
    private static string FormatCellValue(object? value, Type propertyType)
    {
        ...
        if (type == typeof(bool))
            return (bool)value
                ? KescoGridPrintStyles.BoolTrueIcon
                : KescoGridPrintStyles.BoolFalseIcon;
    ...
    private static void EmbedStyles(StringBuilder sb)
    {
        ...
        sb.Append(".mud-main-content>*:not(.kesco-grid-printing){display:none!important}");
        ...
        // Скрыть тулбар
        sb.Append(".kesco-grid-printing .kesco-grid-search,");
        sb.Append(".kesco-grid-printing .toolbar-columns-btn,.kesco-grid-printing .toolbar-add-btn,");
        sb.Append(".kesco-grid-printing .toolbar-select-btn,.kesco-grid-printing .toolbar-batch-btn,");
        sb.Append(".kesco-grid-printing .grouping-toggle-btn,.kesco-grid-printing .filter-toggle-btn");
        sb.Append("{display:none!important}");

        // Скрыть пагинатор
        sb.Append(".kesco-grid-printing .kesco-grid-paginator{display:none!important}");

        // Скрыть чекбоксы
        sb.Append(".kesco-grid-printing .mud-table-head .mud-table-cell:has(.mud-checkbox),");
        sb.Append(".kesco-grid-printing .mud-table-body .mud-table-cell:has(.mud-checkbox){display:none!important}");

        // Скрыть кнопки закрытия на чипах
        sb.Append(".kesco-grid-printing .chip-remove-btn{display:none!important}");

        // Треи скрыты, описания показаны
        sb.Append(".kesco-grid-printing .grouping-tray,.kesco-grid-printing .filter-tray,");
        sb.Append(".kesco-grid-printing .grouping-tray--active,.kesco-grid-printing .filter-tray--active{display:none!important}");
        sb.Append(".kesco-grid-print-descriptions{display:none}");
        sb.Append(".kesco-grid-printing .kesco-grid-print-descriptions{display:block!important;margin-bottom:8px!important}");
        sb.Append(".kesco-grid-printing .kesco-grid-print-group-desc,");
        sb.Append(".kesco-grid-printing .kesco-grid-print-filter-desc{font-size:8pt!important;color:#555!important;margin-bottom:4px!important}");

        // Заголовок
        sb.Append(".kesco-grid-printing .mud-typography-h5{display:block!important;font-size:16pt!important;");
        sb.Append("font-weight:bold!important;color:#000!important;margin-bottom:12px!important}");

        // MudPaper
        sb.Append(".kesco-grid-printing.mud-paper{box-shadow:none!important;border:none!important;");
        sb.Append("border-radius:0!important;padding:8px!important}");

        // Контейнер таблицы
        sb.Append(".kesco-grid-printing .mud-table-container{overflow:visible!important;");
        sb.Append("height:auto!important;max-height:none!important}");

        // Таблица
        sb.Append(".kesco-grid-printing .mud-table{background:#fff!important;width:100%!important}");

        // Хедер — тёмный
        sb.Append(".kesco-grid-printing .mud-table-head .mud-table-cell{background:#222!important;");
        ...
        // Ячейки данных
        sb.Append(".kesco-grid-printing .mud-table-body .mud-table-cell{border-bottom:1px solid #ccc!important;");
        sb.Append("border-right:1px solid #eee!important;color:#000!important;background:#fff!important}");

        // Убрать hover/striped
        sb.Append(".kesco-grid-printing .mud-table-body .mud-table-row:hover{background:transparent!important}");
        sb.Append(".kesco-grid-printing .mud-table-body .mud-table-row:hover .mud-table-cell{background:#fff!important}");
        sb.Append(".kesco-grid-printing .mud-table-striped .mud-table-row:nth-child(even) .mud-table-cell{background:#fff!important}");

        // Групповые строки
        sb.Append(".kesco-grid-printing .mud-table-row:has(.group-header-cell){background:#fff!important;");
        sb.Append("border-top:2px solid #000!important}");
        sb.Append(".kesco-grid-printing .group-header-cell{color:#000!important;font-weight:bold!important}");

        // Thead на каждой странице
        sb.Append(".kesco-grid-printing .mud-table-head{display:table-header-group}");
        sb.Append(".kesco-grid-printing .mud-table-row{page-break-inside:avoid}");
