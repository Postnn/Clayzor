...
/// <summary>
/// Генератор Excel-файлов (.xlsx) для данных грида Kesco.
/// Использует ClosedXML для создания стилизованных книг с цветовой гаммой Kesco.
/// </summary>
public static class KescoGridExcelGenerator
{
    // ── Цветовая гамма и типографика Kesco ────────────────────────────────────
    private const string KescoFontFamily = "Verdana";

    private static readonly XLColor KescoNavy  = XLColor.FromHtml("#05164D");
    private static readonly XLColor KescoGold  = XLColor.FromHtml("#FFAD00");
    ...
    private static void WriteTitleRow(IXLWorksheet ws, int rowNum, string title, int colCount)
    {
        ...
        range.Style.Font.FontName = KescoFontFamily;
        range.Style.Fill.BackgroundColor = KescoNavy;
        range.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
        range.Style.Alignment.Vertical = XLAlignmentVerticalValues.Center;
        // Золотое подчёркивание
        range.Style.Border.BottomBorder = XLBorderStyleValues.Medium;
        range.Style.Border.BottomBorderColor = KescoGold;
    ...
    private static void WriteHeaderRow(IXLWorksheet ws, int rowNum, IReadOnlyList<KescoColumnMeta> columns)
    {
        var row = ws.Row(rowNum);
        row.Height = 24;
        ...
        for (int c = 0; c < columns.Count; c++)
        {
            ...
            cell.Style.Fill.BackgroundColor = KescoNavy;
    ...
    private static void WriteGroupHeaderRow(IXLWorksheet ws, int rowNum, GroupHeaderRow header, int colCount)
    {
        ...
        cell.Style.Font.FontName = KescoFontFamily;
        cell.Style.Font.FontColor = KescoNavy;
