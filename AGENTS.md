...
- `ValueFilter` реализует `IKescoFilterNode` — листовой узел дерева: фильтрация по набору выбранных значений (Excel-style). Поля: `Column`, `Values`, `Negate` (IN/NOT IN), `BlankChecked` (NULL/пустые строки), `ParamPrefix`. В одной колонке одновременно активен либо `ColumnFilter`, либо `ValueFilter` — они взаимоисключающие. SQL-генерация — V2
