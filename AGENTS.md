...
- **Print**: печать текущей страницы — `kescoGridPrint.printCurrentPage()` (добавляет `.kesco-grid-printing` на грид → `window.print()` → `afterprint`). Печать всех данных — `kescoGridPrint.printHtml(html)` (рендерит HTML в скрытый iframe, печатает iframe, удаляет после `afterprint`). `PrintAllInternal()` вызывает `DataLoader.BuildPrintHtmlAsync()`, который через `BuildAllRowsForPrint()` загружает все строки без пагинации и через `KescoGridPrintHtmlGenerator.Build()` генерирует HTML. **HTML использует те же MudBlazor CSS-классы** (`.mud-table`, `.mud-table-cell`, `.group-header-cell`) и встраивает полный `@media print` CSS — результат визуально идентичен печати текущей страницы. Грид **не затрагивается**. На время загрузки — спиннер (`_isExporting`). Печать «Выбранных» — заглушка
