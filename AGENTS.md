...
- **Tray borders & icons**: панели группировки (`.grouping-tray`) и фильтрации (`.filter-tray`) имеют идентичное оформление: `border-left: 3px solid var(--lh-navy)`, `border-bottom: 2px solid var(--lh-gold)`. Иконки обеих панелей — `color: var(--lh-navy)`, `opacity: 0.45`. При наведении/перетаскивании `border-left-color` меняется на `var(--lh-gold)`. CSS определён в `wwwroot/css/app.css`
