...
- **Print**: печать текущей страницы реализована через `kescoGridPrint.js` — добавляет класс `.kesco-grid-printing` на целевой грид перед `window.print()` и снимает его после закрытия диалога печати. Стили печати — `@media print` в `app.css`: скрывают layout chrome (appbar, drawer, overlay), показывают только грид с классом `.kesco-grid-printing`, скрывают тулбар/пагинатор/чекбоксы, форматируют таблицу в Ч/Б (тёмный header, светлые строки), печатают `thead` на каждой странице, `@page { size: landscape; margin: 15mm }`. Печать «Всех данных» и «Выбранных» — заглушки
