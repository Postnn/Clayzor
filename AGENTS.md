...
| **KescoGridPrintHtmlGenerator** — статический генератор самодостаточного HTML-документа для печати всех данных грида через скрытый iframe. Инлайн-стили (Ч/Б-таблица, тёмный header, `@page{landscape;15mm}`, `thead{display:table-header-group}`) | [docs/kesco-grid.md](docs/kesco-grid.md) |
...
- **Print**: печать текущей страницы — `kescoGridPrint.printCurrentPage()` (добавляет `.kesco-grid-printing` на грид → `window.print()` → `afterprint`). Печать всех данных — `kescoGridPrint.printHtml(html)` (рендерит HTML в скрытый iframe, печатает iframe, удаляет после `afterprint`). `PrintAllInternal()` в гриде вызывает `DataLoader.BuildPrintHtmlAsync(columns, title, ...)`, который через `BuildAllRowsForPrint()` загружает все строки без пагинации (плоский режим: `SELECT * FROM _src WHERE ... ORDER BY ...`; группировка: `WalkTree(1, int.MaxValue)` + все detail-строки развёрнутых групп) и через `KescoGridPrintHtmlGenerator.Build()` генерирует самодостаточный HTML с инлайн-стилями (Ч/Б-таблица, тёмный header, `@page{landscape;15mm}`, `thead{display:table-header-group}`). Грид (`_rows`, `_dataKey`, `_query`) **не затрагивается** — печать полностью изолирована. На время загрузки показывается `MudProgressCircular` (`_isExporting`). Печать «Выбранных» — заглушка
- **Excel export**: экспорт в Excel реализован через `KescoGridExcelGenerator` (генератор .xlsx на сервере, ClosedXML) и `kescoGridExcel.js` (скачивание на клиенте через Blob URL). Генератор создаёт стилизованную книгу в цветах Kesco (navy `#05164D`, gold `#FFAD00`, серый stripe `#F2F4F7`) со шрифтом Verdana. Поддерживает заголовок грида, описания фильтров/группировки, групповые строки с Excel Outline (сворачиваемые группы), авто-ширину колонок. На время экспорта рядом с заголовком грида показывается `MudProgressCircular` (Size.Small, Indeterminate) — флаг `_isExporting` управляется в `try/finally` каждого из трёх методов (`ExcelCurrentPageInternal`, `ExcelAllInternal`, `ExcelSelectedInternal`). Экспорт «Всех данных» — полностью реализован через `BuildAllRowsForExcel()`: в плоском режиме один запрос `SELECT * FROM _src`, в режиме группировки — два запроса (GROUP BY для агрегатов + плоский SELECT всех строк, групповая структура строится в C# детектированием смены ключа). Экспорт «Выбранных» — TODO (пока выгружает текущую страницу)
