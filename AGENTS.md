...
| `KescoGrid.Filtering.cs` | ~420 | `_filterRoot`, `HasComposite`, `ValueFilterLeaves` (V12), `_valueFilterDisabledColumns` (V8), `OpenFilterDialog` (+`initialOperator`), `OpenValueFilterDialog` (V7), `ApplyValueFilter`, `RemoveValueFilter`, `DescribeValueFilter` (V8), `BuildCurrentQuery`, `OpenCompositeFilterDialog`, чипы, фильтр-трей |
