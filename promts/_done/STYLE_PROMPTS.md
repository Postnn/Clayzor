# Clayzor — промты для внедрения единого стиля
...
- Clayzor.App.Web.MedicalTests (приложение)
- Clayzor.Lib.Web.Controls (библиотека контролов: ClayGrid, ClayButton, диалоги)

Единственные штатные места визуального стиля:
- Clayzor.App.Web.MedicalTests/wwwroot/css/app.css
- Clayzor.Lib.Web.Controls/Themes/ClayTheme.cs (тема MudBlazor)
...
который НЕ понимает var(...). Присвоение Primary = "var(--clay-navy)" роняет приложение
(ArgumentException: Not a valid color на ClayTheme.Create()). Поэтому цвета — литералы в
C#, а app.css их потребляет через эмитируемые MudBlazor переменные --mud-palette-*.

Задача: убрать дублирование цвета между app.css и ClayTheme.cs, сделав единственным
источником тему. Значения цветов не менять.

Шаги:
1. Создай Clayzor.Lib.Web.Controls/Themes/ClayColors.cs — static-класс с public const
...
2. В ClayTheme.cs собери PaletteLight из ClayColors.* (ЛИТЕРАЛЫ, не var()):
       Primary = ClayColors.Navy, Secondary = ClayColors.Gold,
       Error = ClayColors.Error, Info = ClayColors.BlueMid, ... — все поля.
   Приватные const-строки цветов, дублирующие ClayColors, удали.
   Typography.FontFamily = ["var(--clay-font-family)"] ОСТАВЬ — там var() валиден.
...
3. В app.css блок :root замени литералы цвета на алиасы к палитре MudBlazor (её
   ...
   Не-цветовые константы оставь литералами в :root:
       --clay-font-family, --clay-font-size (0.75rem), --clay-radius (2px),
       --clay-accent (3px), --clay-white (#FFFFFF).
4. Существующие --lh-* НЕ переписывай по файлу — сделай алиасами на --clay-*:
       --lh-navy: var(--clay-navy); --lh-gold: var(--clay-gold); ... (все девять).
       --lh-navy-light в файле не используется — можно алиас на --clay-navy-2.
...
Не меняй app.js, разметку компонентов и бизнес-логику. Только ClayColors.cs, палитра в
ClayTheme.cs и блок :root в app.css.
```
...
```
Задача: сделать так, чтобы визуальная стилизация вне app.css/ClayTheme.cs приводила
...
6. Создай проект-анализатор Clayzor.Lib.Web.Controls.Analyzers (netstandard2.0,
   Microsoft.CodeAnalysis.CSharp) с диагностикой CLAY001 (severity Error):
   "Визуальная стилизация вне app.css/ClayTheme.cs запрещена".
7. Анализатор проверяет строковые литералы и интерполяции: если строка содержит
   визуальный CSS (те же паттерны, что в п.3) ИЛИ подстроку "<style" — репортит CLAY001
...
9. Добавь юнит-тесты анализатора: (а) хардкод "background:#d32f2f" → CLAY001;
...
1. Clayzor.Lib.Web.Controls/Components/ClayErrorBar.razor — ХУДШИЙ случай.
   Полностью самописная разметка с хардкод-цветами (#d32f2f, #b71c1c, #8b0000, #ffcdd2…).
   - Перепиши на MudBlazor (MudAlert/MudPaper) ИЛИ на семантические классы .clay-error-*.
   - Все красные цвета замени на var(--mud-palette-error) / var(--mud-palette-error-darken)
     и производные — определи эти классы (.clay-error-bar, .clay-error-details,
     .clay-error-sql, .clay-error-param-table) в app.css.
   - В .razor должны остаться только Class="…", без единого визуального style=.

2. Clayzor.App.Web.MedicalTests/Components/Pages/Home.razor
   ...
   - Ввести классы .clay-card (нейтральная карточка) и .clay-card--accent
     (золотая левая граница) в app.css; в разметке — Class="pa-5 clay-card--accent".
   - Цвет вторичного текста — классом (.clay-text-secondary { color: var(--mud-palette-text-secondary) }).
   - Иконки-акценты — через Color="Color.Primary"/"Color.Secondary", не Style.

3. Clayzor.App.Web.MedicalTests/Components/Layout/MainLayout.razor
   - Style="font-weight:700; letter-spacing:0.04em;" на заголовке → класс .clay-appbar-title.
   - Style="opacity:0.85;" → класс (opacity — не визуальный цвет, но вынеси для чистоты).
   - Style="border-color:rgba(255,255,255,0.2);" на MudDivider → класс .clay-appbar-divider
     с цветом из токена.
   - Style="color:#8A93A8; font-size:0.65rem;" в футере drawer → класс .clay-drawer-version
     (цвет из --mud-palette-text-secondary или --clay-grey-mid).

4. Clayzor.Lib.Web.Controls/Services/ClayGridPrintHtmlGenerator.cs
   - Если генерирует CSS сам — перенеси весь CSS в ClayGridPrintStyles.cs (белый список),
     оставив в генераторе только вызов/вставку готовой строки стилей.
   - Значения цветов в ClayGridPrintStyles.cs синхронизировать с --clay-* (общие C#-константы).

5. Прочий инлайн в гриде и диалогах (ClayColumnSettingsDialog.razor, ClayGrid.razor,
   ClayCheckbox.razor, ClayColumnValueFilterDialog.razor, фильтры):
...
var(--clay-*) (бренд), и только в app.css/ClayTheme.cs. Нужен новый вид элемента —
...
## Промт 5 — размер лейбла/легенды input-полей = --clay-font-size

```
Контекст: в app.css правило legend { font-size: var(--clay-font-size) } УЖЕ есть, но
...
var(--clay-font-size) в обоих состояниях (в покое и всплывшими). Правки ТОЛЬКО в
Clayzor.App.Web.MedicalTests/wwwroot/css/app.css. Инлайн-стили не добавлять.
...
1. В app.css замени существующий блок правил лейбла/легенды (около строк 63–67) на:
   ...
   .mud-input-control .mud-input-label-outlined {
       font-size: var(--clay-font-size) !important;
   }
...
2. Запусти приложение, открой форму MedicalTestEditDialog и проверь ЛЮБОЕ outlined-поле
   (MudTextField, MudNumericField, ClayComboBox). Всплывший лейбл должен быть размером
...
4. ОБЯЗАТЕЛЬНО: на .mud-input в форме висит класс mud-typography-subtitle1 — у обычного
   ...
   размера --clay-font-size, но сидит одна в рамке, рассчитанной на крупный текст.
   ...
   - Найди источник mud-typography-subtitle1 (Typo=/Class= на поле в .razor или в обёртке
     ClayEditForm/ClayComboBox — вероятно, остаток прошлых правок) и убери его.
   - Убедись, что в загруженном app.css есть и работает:
     .mud-input-control input, .mud-input-control .mud-input-slot { font-size:
     var(--clay-font-size) !important }
   После этого рамка и значение сядут на --clay-font-size, и подпись в пустом поле
   перестанет теряться — размер станет одинаковым во всех состояниях.
5. Заодно устрани нарушение конституции: в Clayzor.Lib.Web.Controls/Components/
   ClayComboBox.razor есть инлайн <style> с .clay-combo-popover. Перенеси эти правила
...
6. Верификация в DevTools:
   - Выбери <label class="… mud-input-label-outlined">, в Computed проверь font-size =
     значение --clay-font-size (напр. 12.8px при 0.8rem) и что в transform НЕТ scale < 1.
...
```
Задача: уплотнить форму src/Clayzor.App.Web.MedicalTests/Components/Pages/
...
ЭТАЛОН — Clayzor.Lib.Web.Controls/Components/Grid/Filter/ClayFilterValueEditor.razor:
...
2. Комбобокс приведи к тому же виду: в Clayzor.Lib.Web.Controls/Components/
   ClayComboBox.razor у внутреннего MudSelect добавь Margin="Margin.Dense" Dense="true"
...
классом-обёрткой формы (например обернуть поля в <div class="clay-form-dense">):
7. .clay-form-dense .mud-input-control-helper-container { min-height: 0; }
   .clay-form-dense .mud-input-control-helper-text { margin-top: 2px; line-height: 1.2; }
